...
- **Phase:** **Coding Arena (Pillar 2) COMPLETE — browser + API verified.** Self-hosted Piston sandbox runs Python/C/C++/JS; 3 pattern-tagged stdin/stdout problems (public+hidden tests); `/arena` UI (list + Monaco + language switch + Run/Submit); Run shows per-test tracebacks; Submit runs all tests + gpt-5 review (Big-O, correctness, suggestions) + persists a submission; **tiered non-spoiler hints** (3 levels); **progress + spaced-repetition** (submissions/reviews tables, Solved/Review-due badges, SM-2-lite schedule). MVP core (P0–P6) + persistence also complete.
- **Next action:** Other pillars — **Live Mock Coding Interview** (bare editor + AI watching) and **Behavioral Voice Round** — or **P7 polish/deploy** (README + architecture diagram + live URL).
