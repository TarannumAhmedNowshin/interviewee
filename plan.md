...
- **Phase:** **DB persistence + replay done (browser-verified).** Sessions, turns (with stage/move), diagrams, and reports persist to Postgres (best-effort — a DB hiccup never blocks a live interview). A History list + read-only replay (transcript + whiteboard snapshot + scored debrief) render from the DB. MVP core (P0–P6) + persistence are complete.
- **Next action:** **P7 polish / deploy** (editorial UI, curated prompt bank, README + architecture diagram, live deploy) — or start the **other pillars** the user asked for (coding arena, live mock coding, behavioral).
- **Watch-outs:** (1) Whisper **3 RPM** limit. (2) Postgres runs on **port 5433** (5432 was taken by another project's container). (3) VAD energy threshold may need tuning. (4) React StrictMode double-mounts the WS in dev (harmless "closed before established" warning).
