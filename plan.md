...
- **Phase:** **Live Mock Coding Interview (Pillar 3) COMPLETE — API verified.** Timed 25-min live round: **bare Monaco editor** (autocomplete/suggestions/squiggles disabled) + AI interviewer that watches the code over a WebSocket and probes approach, complexity, and edge cases; two-model design (gpt-5-mini director picks stage+move aware of time remaining; gpt-5 interviewer streams spoken replies) reusing the voice pipeline (TTS + Whisper STT + client VAD + barge-in); countdown auto-ends → gpt-5 rubric debrief (approach, correctness, code quality, complexity, communication, time management); persisted to `mock_sessions`/`mock_turns` with a `/mock` lobby + room + replay. Also done: Coding Arena (Pillar 2), MVP core (P0–P6) + persistence.
...
- **Watch-outs:** (1) Whisper 3 RPM. (2) Postgres on **port 5433**. (3) Piston needs `NODE_TLS_REJECT_UNAUTHORIZED=0` (dev-only) for its GitHub runtime downloads behind corporate TLS. (4) Piston max `run_timeout`=3000ms. (5) PowerShell is in **ConstrainedLanguage mode** — no .NET method calls (`.Substring`, `[Math]`) in terminal commands. (6) Typing into Monaco via automation is unreliable.
