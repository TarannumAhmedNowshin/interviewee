...
- **Phase:** **ALL FOUR PILLARS COMPLETE + browser-verified.** Coding Arena (P1), System Design Voice Room (P2), Live Mock Coding (P3), and **Behavioral Voice Round (P4)** all work end-to-end. This pass also added: **session rehydration** (async `get_or_create` + DB restore so a backend restart resumes a live interview), **unified history** (one list across all modes with type badges), an **automated pytest suite** (16 tests, offline), and a **fully ruff-clean backend**.
- **Next action:** **P7 polish/deploy** (README + architecture diagram + live Azure URL) and/or cross-cutting features (résumé/JD personalization, analytics dashboard, RAG + company-style banks, auth). Voice (STT/TTS + barge-in) across all rooms still needs a human mic/speaker pass.
- **Watch-outs:** (1) Whisper 3 RPM. (2) Postgres on **port 5433**. (3) Piston needs `NODE_TLS_REJECT_UNAUTHORIZED=0` (dev-only) for its GitHub runtime downloads behind corporate TLS. (4) Piston max `run_timeout`=3000ms. (5) PowerShell is in **ConstrainedLanguage mode** — no .NET method calls (`.Substring`, `[Math]`) in terminal commands. (6) Monaco has no `.inputarea` in this build — automate code entry via `window.monaco.editor.getModels()[0].setValue(...)`. (7) A network blip auto-resumes; a full backend restart now rehydrates live context from the DB.
...
- **2026-08-22** — **Pillar 4 (Behavioral Voice Round) COMPLETE + platform hardening.** Built the fourth and final pillar: a STAR-method spoken behavioral interview. Backend: `behavioral_questions.py` (8-question bank), `behavioral_interviewer.py` (STAR stages/moves + interviewer/director/scoring prompts, 5-dim rubric: structure, ownership, impact, communication, reflection), `behavioral_orchestrator.py`, `behavioral_ws.py` (`/ws/behavioral/{id}` — reuses TTS+STT+VAD+barge-in, no code channel), `behavioral.py` (`/behavioral/questions|questions/{id}|sessions|sessions/{id}`). DB: `behavioral_sessions`/`behavioral_turns` + repo helpers. Frontend: `lib/behavioral.ts`, `lib/useBehavioral.ts` (voice-first hook with the full reconnect robustness), `/behavioral` lobby + `/behavioral/[id]` room (question + STAR guide + transcript, no editor) + `/behavioral/replay/[id]`; "Behavioral" wired into every nav. **Also:** (1) **Session rehydration** — `get_or_create` is now async across all three orchestrators and restores live context (problem/question + history + stage) from the DB, so a full backend restart mid-interview resumes instead of losing state. (2) **Unified history** — `/history` merges System Design + Mock + Behavioral into one date-sorted list with colored type badges and per-type replay links. (3) **Automated tests** — added `pytest` + `backend/tests/` (16 offline tests: TestClient endpoint/404 coverage + orchestrator banks/helpers/empty-report guards). (4) **Ruff-clean backend** — fixed all remaining `E501`/`UP017` warnings (`ruff check app tests` passes). Browser-verified Pillar 4 end-to-end (lobby → warm opening → `intro·ask_question` → `task·probe_task` → 5-dim STAR debrief 2.0/5 → persisted → shows in unified history).
