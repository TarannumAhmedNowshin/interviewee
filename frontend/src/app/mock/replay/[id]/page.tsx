...
import { getMockSession, type MockSessionDetail } from "../../../../lib/mock";
...
export default function MockReplay({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [data, setData] = useState<MockSessionDetail | null>(null);
      ...
      <header className="flex items-center justify-between border-b border-neutral-800 px-6 py-4">
        <div className="min-w-0">
          <h1 className="truncate text-lg font-semibold tracking-tight">
            {data?.problem_title ?? "Replay"}
          </h1>
          <p className="text-sm text-neutral-500">Mock interview replay</p>
        </div>
        <Link href="/mock" className="shrink-0 text-sm text-neutral-400 hover:text-neutral-200">
          ← All rounds
        </Link>
      </header>
            ...
            {data.code && (
              <section>
                <h2 className="mb-3 text-sm font-semibold text-neutral-400">
                  Final code · {data.language}
                </h2>
                <pre className="overflow-x-auto rounded-lg border border-neutral-800 bg-neutral-900 p-4 font-mono text-xs text-neutral-200">
                  {data.code}
                </pre>
              </section>
            )}
